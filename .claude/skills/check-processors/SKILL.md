...
description: Scan Queenswood processor bricks (paired `components/bank-X` + `bases/bank-X-processor`) for violations of the processor-brick conventions. The `fdb`-outside-`store.clj` check runs wider, over every brick that owns a `store.clj` — relays and query bricks have no `commands.clj`, so they are not processors but the rule still binds. Checks: `fdb` requires leaking outside `store.clj` and `watcher.clj`, `watcher.clj` using `fdb` beyond `fdb/ctx->txn`, `domain.clj` requiring `fdb` / store / schema, `error/reject` appearing in `store.clj` / `interface.clj` / `watcher.clj` / `system.clj` (hard fail) or in `core.clj` (advisory watch list), `domain.clj` functions accepting a `txn` parameter, and `schema/X->Y` type-conversion calls leaking into `domain.clj`. Report findings against `docs/tdd/processor-bricks.md`. Use as a per-brick lint ("check processors", "audit processor bricks", "lint processor conventions", "do my processor bricks follow the pattern?", "check processor invariants").
allowed-tools: Bash
---
...
A "processor brick" here means any `components/bank-X` that has
...
- `bash .claude/skills/check-processors/checks.sh <brick> ...` —
  explicit brick names (e.g. `bank-cash-account bank-party`)

## Checks

| Check | What it flags | Source invariant |
|-------|---------------|-------------------|
| `fdb-leak` | a require of `com.repldriven.mono.fdb.interface` in any file other than `store.clj` or `watcher.clj` | TDD: "FDB is required only in `store.clj` and `watcher.clj`" |
| `watcher-fdb-scope` | any `fdb/<symbol>` reference in `watcher.clj` other than `fdb/ctx->txn` | TDD: "The `watcher.clj` carve-out is for `fdb/ctx->txn` exclusively" |
| `domain-impurity` | a require of `com.repldriven.mono.fdb.interface`, `com.repldriven.mono.bank-schema.interface`, or this brick's own `.store` in `domain.clj` | TDD: "`domain.clj` requires no `fdb`, no `store`, no `bank-schema`" |
| `rejection-misplaced` | `error/reject` in `store.clj`, `interface.clj`, `watcher.clj`, or `system.clj` — hard fail. (`commands.clj`, `domain.clj`, `validation.clj` are the sanctioned sites and are not scanned.) | TDD: "Rejections originate in `domain.clj`, with `commands.clj` as a sanctioned site for protocol-level rejections" |
...
- **`fdb-leak`** — move the FDB-touching code into `store.clj`
  ...
  directly.
- **`watcher-fdb-scope`** — same as `fdb-leak`. The only
  acceptable `fdb/` symbol in `watcher.clj` is `fdb/ctx->txn`,
  which adapts the changelog harness's raw `ctx` into a `Txn`.
  Anything else belongs in `store.clj`.
...
- **`rejection-misplaced`** — for `store.clj`, change the
  ...
  `interface.clj` / `watcher.clj` / `system.clj`, refactor the
...
- **`domain-takes-txn`** — refactor the domain fn to take
  ...
  `bank-cash-account/domain.clj` for the pattern).
...
- A header line naming the bricks in scope (e.g.
  `Scope: branch — 2 brick(s): bank-cash-account, bank-party`).
