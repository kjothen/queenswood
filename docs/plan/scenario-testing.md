...
- [x] **1.1** Create `test-model` component skeleton with empty `interface.clj`. Add to root `deps.edn` (`:dev` and `:+bank` aliases) so existing `project:dev` test runs pick it up. No new project — there's nothing here we'd build into a deployable artifact, so the `bank-tests` plumbing lives in the existing test pipeline (the `external-test-runner` base + each brick's `test/`). When the scenario runner needs an entrypoint in Phase 3, it'll be a `bases/test-scenarios` base, not a project. Confirm `clojure -M:poly check` is clean.
...
- [x] **1.4** Implement `:inbound-transfer` spec in `transfers.clj`. Re-implement the relevant policy rule (`available >= 0`, with the lenient `improving` allowance) as a pure helper in `policy.clj`; the model never imports `policy`. State stays unchanged when the helper returns `:denied`.
...
- [x] **1.8** Unit tests in `test/com/repldriven/queenswood/test_model/` for each command's `:next-state` in isolation. Use `clojure.test/deftest` here — these are pure-function tests, the one place deftest is appropriate.
...
- [x] **2.1** Create `test-projections` component skeleton. Depend on `balance/interface`.
...
- [x] **3.1** Create `test-scenarios` component skeleton. Depend on `test-model/interface`, `test-projections/interface`, and the production command-submission interface.
...
- [x] **4.5** Author three scenarios in `components/test-scenarios/test-resources/bank-test-scenarios/scenarios/`:
...
- [x] **5.1** Add `org.clojure/test.check` and `io.vouch/fugato` to `test-scenarios` test deps. Fugato is unreleased on Clojars; pulled as a git dep pinned to `vouch-opensource/fugato` SHA `4c1f687b`.
...
- [x] **6.4** Capstone: full UI happy-path encoded as `full-happy-path.edn`. 39 steps: org + settlement, two products (current + savings @ 3650 bps), Arthur + Ford as person-parties, four customer accounts, settlement funded with £50,000, party rewards, intra-party current↔savings moves, an outbound payment, daily accrual, monthly capitalisation. Asserts every key balance pre- and post-interest. The scenario landed two side findings:
  ...
  - The cash-account opening transition (`:opening` → `:opened`) goes through a changelog watcher in production. Without that watcher in the test system, freshly-opened accounts stay `:opening` and `interest/accrue-daily` silently filters them out. Added `cash-account/seed-opened-account` (test-bypass, parallels `party/seed-active-party`) and the runner now flips status inline at open-time.
...
- **Resist enriching the model.** Throughout, the model stays pure-functional and ignorant of production infrastructure. If `test-model` starts importing proto schemas or Malli contracts, push back.
