  ...
  (testing "Creating a service account returns the account details"
    (with-test-system
     ...
     (let [db (system/instance sys [:migrator :migrations])]
       (nom-test> [result (SUT/create-service-account db
                                                      project-name
                                                      "sa-test"
                                                      "Test Service Account"
                                                      "A test service account")
                   _ (is (= sa-id (:id result)))
                   _ (is (= "sa-test" (:name result)))
                   _ (is (= "prj-test" (:project-id result)))
                   _ (is (= sa-email (:email result)))
                   _ (is (= "Test Service Account" (:display-name result)))
                   _ (is (= "A test service account" (:description result)))
                   _ (is (= false (:disabled result)))])))))
...
(deftest get-service-account-test
  (testing "Getting a service account by name returns the account"
    (with-test-system [sys "classpath:iam/application-test.yml"]
                      (let [db (system/instance sys [:migrator :migrations])]
                        (nom-test> [_ (SUT/create-service-account
                                       db
                                       project-name
                                       "sa-test"
                                       "Test Service Account"
                                       "A test service account")
                                    result (SUT/get-service-account db sa-id)
                                    _ (is (= sa-id (:id result)))
                                    _ (is (= sa-email (:email result)))])))))
...
(deftest get-service-account-by-unique-id-test
  (testing "Getting a service account by unique-id returns the account"
    (with-test-system
     [sys "classpath:iam/application-test.yml"]
     (let [db (system/instance sys [:migrator :migrations])]
       (nom-test> [created (SUT/create-service-account db
                                                       project-name
                                                       "sa-test"
                                                       "Test Service Account"
                                                       "A test service account")
                   result (SUT/get-service-account
                           db
...
(deftest list-service-accounts-test
  (testing "Listing service accounts returns accounts for the project"
    (with-test-system [sys "classpath:iam/application-test.yml"]
                      (let [db (system/instance sys [:migrator :migrations])]
                        (nom-test> [_ (SUT/create-service-account db
                                                                  project-name
                                                                  "sa-one"
                                                                  "Account One"
                                                                  "")
                                    _ (SUT/create-service-account db
                                                                  project-name
                                                                  "sa-two"
                                                                  "Account Two"
                                                                  "")
                                    result
                                    (SUT/list-service-account db project-name)
                                    _ (is (= 2 (count result)))])))))
    ...
    (with-test-system
     ...
     (let [db (system/instance sys [:migrator :migrations])]
       (nom-test> [_ (SUT/create-service-account db
                   ...
                   _ (SUT/patch-service-account db
                                                sa-id
                                                "Updated Name"
                                                "Updated description")
                   result (SUT/get-service-account db sa-id)
                   _ (is (= "Updated Name" (:display-name result)))
                   _ (is (= "Updated description" (:description result)))])))))
    ...
    (with-test-system [sys "classpath:iam/application-test.yml"]
                      (let [db (system/instance sys [:migrator :migrations])]
                        (nom-test> [_ (SUT/create-service-account
                                       ...
                                       "Test Service Account"
                                       "")
                                    _ (SUT/disable-service-account db sa-id)
                                    disabled (SUT/get-service-account db sa-id)
                                    _ (is (= true (:disabled disabled)))
                                    _ (SUT/enable-service-account db sa-id)
                                    enabled (SUT/get-service-account db sa-id)
                                    _ (is (= false (:disabled enabled)))])))))
    ...
    (with-test-system
     ...
     (let [db (system/instance sys [:migrator :migrations])]
       (nom-test> [_ (SUT/create-service-account db
                                                 ...
                                                 "Test Service Account"
                                                 "")
                   result (SUT/delete-service-account db sa-id)
                   _ (is (= sa-id (:id result)))
                   _ (is (nil? (SUT/get-service-account db sa-id)))])))))
