...
(defn- test-system
  []
  (error/nom-> (env/config "classpath:pulsar-vault-crypto/application-test.yml" :test)
               system/defs
               system/start))
...
(deftest pulsar-vault-crypto-test
  (system/with-system [sys (test-system)]
    (let [producer   (system/instance sys [:pulsar :producers :user])
          ...
          schema     (pulsar/schema->avro (get-in schemas [:user :schema]))
      ...
      (testing "Consumer with correct tenant key reads from vault and decrypts"
        (doseq [msg msgs]
          (pulsar/send producer msg {"properties" props}))
        (let [{:keys [c stop]} (pulsar/receive consumer-1 schema 50)
          ...
          (when recv-msgs
            ...
            (is (= msgs (mapv :data recv-msgs)) "Messages should decrypt correctly"))))
        ...
        (let [{:keys [c stop]} (pulsar/receive consumer-2 schema 50)
              ...
              [recv-msgs _]    (async/alts!!
                                [(async/into [] (async/take (count msgs) c))
                                 timeout])]
          (async/>!! stop :stop)
          (is (some? recv-msgs) "Should receive messages (encrypted, undecryptable)")
          (when recv-msgs
      ...
      (testing "Key rotation: updated vault secret is picked up without restart"
        ...
        (let [vault-client (system/instance sys [:vault :client])
              new-pubkey   (get-in (system/instance sys [:pulsar :schemas]) [:user]) ;; placeholder
              ]
          ;; rotation test to be implemented once vault write-secret is available
