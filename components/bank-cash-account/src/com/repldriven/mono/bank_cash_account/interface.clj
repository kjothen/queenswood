(ns com.repldriven.mono.bank-cash-account.interface
  "Cash account write side: open, close, suspend, reopen, and rotate
  the payment-address for banks. Open allocates payment addresses,
  ...
  no watcher leg. Suspend/reopen move between `:cash-account-status-opened`
  and `:cash-account-status-suspended`; rotate-address leaves status
  unchanged.
  ...
  release. A rotated-away payment-address is retired the same way,
  onto the account's `:retired-payment-addresses` history — inbound
  payments to it fall through to the existing suspense path, there is
  no redirect window.
...
(defn rotate-address
  "Rotate an opened account's payment-address: retires the current one
  onto `:retired-payment-addresses` and issues a fresh one from the
  fountain, rewriting `:bban` to match. Direct single-phase flip, no
  watcher leg — the account's status doesn't change. Returns the
  updated account or an anomaly.
