(ns ^:eftest/synchronized com.repldriven.mono.bank-cash-account.interface-test
  (:require com.repldriven.mono.bank-cash-account.interface
...
(defn- poll-status
  ...
  (loop [attempts 50]
    (let [result (send-command proc
      ...
      (cond (= expected (:account-status decoded)) decoded
...
(defn- seed-published-product-version
  ...
  ([record-db store-fn product-id]
   (seed-published-product-version record-db store-fn product-id []))
  ...
  ([record-db store-fn product-id allowed-currencies balance-products]
   (fdb/transact record-db
                 store-fn
                 "cash-account-product-versions"
                 (fn [store]
                   (let [version {:organization-id test-org-id,
...
(defn- test-open-account
  ...
  payment addresses, and balances from product"
    (let [party-id "cust-1"
      ...
      (nom-test>
        [result (send-command proc schemas "open-cash-account" open-payload) _
         (is (= "ACCEPTED" (:status result))) decoded
         (decode-payload schemas "cash-account" result) _
         (is (some? (:account-id decoded))) _
         (is (= :cash-account-status-opened (:account-status decoded))) _
         (is (= test-product-id (:product-id decoded))) _
         (is (string? (:version-id decoded))) _
         (is (= open-payload (select-keys decoded (keys open-payload)))) _
         (is (= "040004"
                (get-in decoded [:payment-addresses 0 :scan :sort-code]))) _
         (is (some? (get-in decoded
                            [:payment-addresses 0 :scan :account-number])))
         account-balances
         (balances/get-balances fdb-config (:account-id decoded)) _
         (is (= 3 (count account-balances))) _
         (is (= #{:balance-type-default}
                (set (map :balance-type account-balances)))) _
         (is (= #{:balance-status-posted :balance-status-pending-incoming
                  :balance-status-pending-outgoing}
                (set (map :balance-status account-balances)))) _
         (is (every? #(= "USD" (:currency %)) account-balances)) _
         (is (every? #(= 0 (:credit %)) account-balances))]))))
    ...
    (let [party-id "cust-2"
      ...
      (nom-test>
        [opened (send-command proc schemas "open-cash-account" open-payload)
         account (decode-payload schemas "cash-account" opened) close-data
         (select-keys account [:organization-id :account-id]) result
         (send-command proc schemas "close-cash-account" close-data) _
         (is (= "ACCEPTED" (:status result))) decoded
         (decode-payload schemas "cash-account" result) _
         (is (= open-payload (select-keys decoded (keys open-payload))))]))))
...
(defn- test-watcher-transitions
  ...
  (testing "watcher transitions closing->closed"
    (let [party-id "cust-watcher"
      ...
      (nom-test>
        [opened (send-command proc schemas "open-cash-account" open-payload)
         account (decode-payload schemas "cash-account" opened) account-id
         (:account-id account) _ (is (= :cash-account-status-opened (:account-status account)))
         closed
         (send-command proc
                       schemas
                       "close-cash-account"
                       {:organization-id test-org-id, :account-id account-id})
         closing-account (decode-payload schemas "cash-account" closed) _
         (is (= :cash-account-status-closing (:account-status closing-account))) polled-closed
         (poll-status proc schemas account-id :cash-account-status-closed) _
         (is (= :cash-account-status-closed (:account-status polled-closed)))]))))
  ...
  (testing "get-cash-account returns account"
    (let [party-id "cust-7"
      ...
      (nom-test> [opened
                  ...
                  _ (is (= "ACCEPTED" (:status result))) decoded
                  (decode-payload schemas "cash-account" result) _
                  (is (= (:account-id account) (:account-id decoded))) _
                  (is (= :cash-account-status-opened (:account-status decoded)))]))))
    ...
    (let [party-id "cust-multi"
      ...
      (nom-test> [r1
                  (send-command proc
                                schemas
                                "open-cash-account"
                                (assoc payload :name "Account A")) _
                  (is (= "ACCEPTED" (:status r1))) r2
                  (send-command proc
                                schemas
                                "open-cash-account"
                                (assoc payload :name "Account B")) _
                  (is (= "ACCEPTED" (:status r2))) a1
                  (decode-payload schemas "cash-account" r1) a2
                  (decode-payload schemas "cash-account" r2) _
                  (is (not= (:account-id a1) (:account-id a2)))]))))
...
(defn- test-open-account-invalid-currency
  ...
  allowed-currencies"
    (let [party-id "cust-currency"
      ...
      (seed-active-party record-db store-fn party-id)
      (seed-published-product-version record-db store-fn product-id ["GBP"])
      (let [result (send-command proc
                                 schemas
                                 "open-cash-account"
                                 {:organization-id test-org-id,
...
(deftest process-cash-accounts-test
  (with-test-system
    ...
    (let [proc (system/instance sys [:cash-accounts :processor])
