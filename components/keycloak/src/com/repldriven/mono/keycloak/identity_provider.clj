(ns com.repldriven.mono.keycloak.identity-provider
  ...
  Domain knowledge stays at the system-config layer: the record's
  `audiences-by-status` map resolves an org's status keyword to the
  client-scope name that gets attached at create-client time. The
  realm import owns the audience mapper for each scope, so tokens
  minted by the resulting client carry the right `aud` claim
  automatically."
  (:require
...
(defrecord KeycloakIdentityProvider [config admin-token jwks
  ...
  identity-provider/IdentityProvider
    (-create-service-account [this {:keys [organization-id name status]}]
      (let [audience (get audiences-by-status status)]
...
(defn ->client
  ...
  `:realm`, `:admin-client-id`, `:admin-client-secret`, and
  `:audiences-by-status` (map from organization-status keyword to
  client-scope/audience name)."
