(ns com.repldriven.mono.server.system
  (:require [com.repldriven.mono.server.jetty :as server-jetty]
  ...
  {:system/start (fn [{:system/keys [config instance]}]
                   (or instance
                       (let [{:keys [handler interceptors options]} config
                             ...
                             server (jetty/run-jetty (handler {:interceptors
                                                                 interceptors})
                                                     options)]
                         (log/info "Jetty listening on"
                                   (server-jetty/http-local-url server))
                         server))),
