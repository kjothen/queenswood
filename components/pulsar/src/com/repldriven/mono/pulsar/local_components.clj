...
(def container-service-url
  {:start (fn [{:keys [container-mapped-ports exposed-port]} instance _]
            (or instance
                (let [service-url-str (str "pulsar://localhost:" (get container-mapped-ports exposed-port))]
...
(def container-service-http-url
  {:start (fn [{:keys [container-mapped-ports exposed-port]} instance _]
            (or instance
                (let [service-http-url-str (str "http://localhost:" (get container-mapped-ports exposed-port))]
