...
(defn- rename-old-create-new-log
  [^File log ^File old-log]
  (.renameTo log old-log)
  (.createNewFile log))

(defn- shift-log-period
  [log path ^Calendar prev-cal]
  (let [postfix (-> "yyyy-MM-dd" SimpleDateFormat. (.format (.getTime prev-cal)))
        ...
        extension (if (>= last-index-of-dot 0) (subs path (inc last-index-of-dot)) "")
...
(defn- log-cal
  [date]
  (let [now (Calendar/getInstance)] (.setTime now date) now))

(defn- prev-period-end-cal
  [date pattern]
  (let [^Calendar cal (log-cal date)
        offset (case pattern
                 :daily 1
  ...
  {:enabled?   true
   ...
   (fn [data]
     (let [{:keys [instant output_]} data
           ...
           ^Calendar prev-cal (prev-period-end-cal instant pattern)]
