  ...
  (with-test-system
    ...
    (let [config (fdb-config sys)]
      (testing "creates a balance with zero totals"
        (nom-test> [balance
                    (SUT/new-balance config
                                     {:account-id "acc_123",
                                      :balance-type :balance-type-default,
                                      :balance-status :balance-status-posted,
                                      :currency "USD"}) _
                    (is (= "acc_123" (:account-id balance))) _
                    (is (= :balance-type-default (:balance-type balance))) _
                    (is (= :balance-status-posted (:balance-status balance))) _
                    (is (= "USD" (:currency balance))) _
                    (is (= 0 (:credit balance))) _ (is (= 0 (:debit balance))) _
                    (is (pos? (:created-at balance))) _
                    (is (pos? (:updated-at balance)))]))
      (testing "loads balance by composite key"
        (nom-test> [result
                    (SUT/get-balance config
                                     "acc_123" :balance-type-default
                                     "USD" :balance-status-posted) _
                    (is (some? result)) _
                    (is (= "acc_123" (:account-id result))) _
      ...
      (testing "lists balances by account"
        (nom-test> [_
                    ...
                    (SUT/get-balances config "acc_123") _
                    (is (= 2 (count balances)))])))))
