...
(defn- test-str-kv
  ...
  (let [db (system/instance sys [:fdb :db])]
    (testing "can store and retrieve string values as raw KV"
      (nom-test> [_ (SUT/set-str db "test-key" "test-value") result
                  (SUT/get-str db "test-key") _
  ...
  (let [whiskers
    ...
    (testing "can store and retrieve Pet records as raw KV"
      (nom-test> [_ (SUT/set-bytes db "pet/1" (test-schema/Pet->pb whiskers))
                  retrieved
                  (error/nom-> (SUT/get-bytes db "pet/1") test-schema/pb->Pet) _
        ...
        [_
         ...
         (SUT/process-changelog record-db
                                ...
                                (fn [_ctx changelog-bytes]
                                  (swap! received conj changelog-bytes))) _
         (is (= 2 (count @received))) _
         (is (= "whiskers-data" (String. ^bytes (first @received)))) _
      ...
      (nom-test>
        [_
         ...
         (SUT/transact record-db
                       ...
                       "pets"
                       (fn [store]
                         (SUT/query-records store "Pet" "species" "hamster"))) _
         (is (= 1 (count results))) retrieved
         (error/nom-> (first results) test-schema/pb->Pet) _
         (is (= whiskers (utility/record->map retrieved)))]))))
