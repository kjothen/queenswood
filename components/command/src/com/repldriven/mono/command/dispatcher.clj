...
(defn start
  ...
  Subscribes once to :command-response on bus. Each arriving
  response is matched to a waiting promise by correlation-id
  and delivered. Handles both keyword (local) and string
  (Pulsar) keys.

  Returns {:bus bus :pending (atom {}) :stop-fn fn}."
  [bus]
  (let [pending (atom {})
             ...
             (fn [response]
               (let [correlation-id (or (:correlation-id response)
                 ...
                 (when-let [p (get @pending correlation-id)]
                   (deliver p response)))))
        stop-fn (fn [] (message-bus/unsubscribe bus :command-response))]
    (if (error/anomaly? sub)
      (throw (ex-info "Failed to start command dispatcher" {:anomaly sub}))
      {:bus bus :pending pending :stop-fn stop-fn})))

(defn stop
  "Stop the dispatcher and unsubscribe from :command-response."
  [{:keys [stop-fn]}]
