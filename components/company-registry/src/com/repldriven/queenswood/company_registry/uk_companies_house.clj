(ns com.repldriven.queenswood.company-registry.uk-companies-house
  (:require
...
(defn fetch-company
  ...
  (let [res (http/request {:method :get
    ...
    (cond
     ...
     (= 404 (:status res))
     (error/reject :company-registry/company-not-found
                   {:message "Company not found"
                    :company-number company-number})

     (>= (:status res) 400)
     (error/fail :company-registry/http
     ...
     (let [body (http/res->edn res)]
       (if (error/anomaly? body)
         (error/fail :company-registry/parse
