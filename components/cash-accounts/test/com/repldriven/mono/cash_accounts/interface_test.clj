(ns ^:eftest/synchronized com.repldriven.mono.cash-accounts.interface-test
  (:require
    ...
    (let [party-id "cust-2"
      ...
      (nom-test>
        [opened (send-command proc
                              ...
                              "open-cash-account"
                              open-payload)
         account (decode-payload schemas
                                 "cash-account"
                                 opened)
...
(defn- test-open-multiple-accounts
  [proc schemas record-db store-fn]
  (testing
    "open-cash-account allows multiple accounts per customer"
...
(defn- test-open-account-no-published-version
  [proc schemas]
  (testing
    "open-cash-account rejects when no published version
  exists"
    (let [result
          (send-command proc
                        schemas
      ...
      (is (error/rejection? result))
      (is (= :cash-account/product-not-published
             (error/kind result))))))

(defn- test-open-account-invalid-currency
  [proc schemas record-db store-fn]
  (testing
    "open-cash-account rejects when currency not in
  allowed-currencies"
      ...
      (let [result
            (send-command proc
                          ...
                          {:organization-id test-org-id
                           ...
                           :product-id product-id})]
        (is (error/rejection? result))
        (is (= :cash-account/invalid-currency
               (error/kind result)))))))
    ...
    (let [result
          (send-command proc
                        ...
                        {:organization-id test-org-id
                         :account-id "acc-8"})]
      (is (error/rejection? result))
      (is (= :cash-accounts/unknown-command
             (error/kind result))))))

(deftest process-cash-accounts-test
  (with-test-system
   [sys "classpath:cash-accounts/application-test.yml"]
   (let [proc (system/instance sys
                               [:cash-accounts :processor])
