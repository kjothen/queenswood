        ...
        (let [schemas (system/instance sys [:avro :serde])]
          (nom-test>
            [result
             (send-command sys
                           "create-pet"
                           {:name "Whiskers", :species "cat", :age-months 24}) _
             (is (= "ACCEPTED" (:status result))) decoded
             (avro/deserialize-same (get schemas "pet") (:payload result)) _
             (is (some? (:pet-id decoded))) _
             (is (= "Whiskers" (:name decoded))) _
             (is (= "cat" (:species decoded)))]))))))
