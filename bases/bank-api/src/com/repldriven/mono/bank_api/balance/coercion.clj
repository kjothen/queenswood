...
(def encode-balance-type
  (let [m (assoc (zipmap (vals balance-types) (map keyword (keys balance-types)))
                 :balance-type-unknown :unknown)]
    (fn [v] (get m v v))))

(def balance-type-json-schema
  {:type "string", :enum (vec (keys balance-types))})
...
(def encode-balance-status
  (let [m (assoc (zipmap (vals balance-statuses)
                 ...
                 :balance-status-unknown :unknown)]
    (fn [v] (get m v v))))
