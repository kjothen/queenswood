...
(defn error-response
  ...
  ([status anomaly]
   (cond-> {:title (cond (error/unauthorized? anomaly) "UNAUTHORIZED"
                         (error/rejection? anomaly) "REJECTED"
                         :else "FAILED"),
            :type (str (error/kind anomaly)),
            :status status}
     (:message (error/payload anomaly)) (assoc :detail
                                          (:message (error/payload anomaly)))))
  ([status command-status result]
   (cond-> {:title command-status, :type (:reason result), :status status}
     (:message result) (assoc :detail (:message result))))
