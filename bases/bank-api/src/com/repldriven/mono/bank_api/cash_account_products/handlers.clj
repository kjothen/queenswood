(ns com.repldriven.mono.bank-api.cash-account-products.handlers
  (:require
    [com.repldriven.mono.bank-api.errors :refer [error-response]]

    [com.repldriven.mono.cash-account-products.interface
...
(defn create-product
  ...
  (let [{:keys [record-db record-store]} request
        ...
        result
        (cash-account-products/new-product
...
(defn create-version
  ...
  (let [{:keys [record-db record-store]} request
        ...
        body (get-in request [:parameters :body])
        result
        (cash-account-products/new-version
         {:record-db record-db :record-store record-store}
         org-id
         product-id
         body)]
    (cond
     ...
     (if (= :cash-account-products/draft-exists
            (error/kind result))
  ...
  (let [{:keys [record-db record-store]} request
    ...
    (cond
     ...
     (let [kind (error/kind result)]
       (cond
        (= :cash-account-products/version-not-found kind)
