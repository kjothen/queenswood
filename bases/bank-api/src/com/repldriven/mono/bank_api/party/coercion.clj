...
(def decode-party-type
  (let [m (merge party-types
                  (zipmap (map keyword (keys party-types))
                          (vals party-types)))]
    (fn [v] (get m v v))))

(def encode-party-type
  (let [m (assoc (zipmap (vals party-types)
                         (map keyword (keys party-types)))
                 :party-type-unknown :unknown)]
    (fn [v] (get m v v))))

(def party-type-json-schema
  {:type "string", :enum (vec (keys party-types))})
...
(def encode-identifier-type
  (let [m (assoc (zipmap (vals identifier-types)
                 ...
                 :identifier-type-unknown :unknown)]
    (fn [v] (get m v v))))
