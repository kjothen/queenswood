(ns com.repldriven.mono.bank-api.parties-test
  (:require [com.repldriven.mono.bank-api.api :as api]
                    ...
                    (let [jetty (system/instance sys [:server :jetty-adapter])
                      ...
                      (binding [*base-url* (server/http-local-url jetty)]
                        (testing "POST /v1/parties sends create-party command"
                          (nom-test>
                            [res (create-party-request) _
                             (is (= 200 (:status res))) body (http/res->edn res)
                             _ (is (= "pty_01TEST123" (:party-id body))) _
                             (is (= "person" (:type body))) _
                             (is (= "Jane Doe" (:display-name body)))])))
                      (stop))))
