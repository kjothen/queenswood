    ...
    (let [record-db (system/instance sys [:fdb :record-db])
      ...
      (binding [*base-url* (server/http-local-url jetty)]
        (testing "lists all accounts"
          (nom-test> [res (list-cash-accounts-request) _
                      (is (= 200 (:status res))) body (http/res->body res) _
                      (is (= 3 (count (get body "cash-accounts")))) _
                      (is (= "acct-001"
        ...
        (testing "paginates with page[size]"
          (nom-test> [res (list-cash-accounts-request "page[size]=2") _
                      (is (= 200 (:status res))) body (http/res->body res) _
                      (is (= 2 (count (get body "cash-accounts")))) _
        ...
        (testing "paginates forward with page[after]"
          (let [after-cursor (cursor/encode "acct-001")]
            (nom-test>
              [res
               (list-cash-accounts-request (str "page[after]=" after-cursor)) _
               (is (= 200 (:status res))) body (http/res->body res) _
               (is (= 2 (count (get body "cash-accounts")))) _
               (is (= "acct-002"
                      (get (first (get body "cash-accounts")) "account-id"))) _
               (is (some? (get-in body ["links" "prev"])))])))
        (testing "paginates backward with page[before]"
          (let [before-cursor (cursor/encode "acct-003")]
            (nom-test> [res
                        (list-cash-accounts-request (str "page[before]="
                                                         before-cursor)) _
                        (is (= 200 (:status res))) body (http/res->body res) _
                        (is (= 2 (count (get body "cash-accounts")))) _
                        (is (= "acct-001"
        ...
        (testing "returns empty when no accounts match"
          (let [after-cursor (cursor/encode "acct-999")]
            (nom-test> [res
                        (list-cash-accounts-request (str "page[after]="
                                                         after-cursor)) _
                        (is (= 200 (:status res))) body (http/res->body res) _
                        (is (= 0 (count (get body "cash-accounts"))))])))))))
