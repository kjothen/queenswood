...
(defn start!
  ...
  (api/init)
  (let [system-config (ring/create-system (get-in @env/env [:system :ring]))]
    (system/start! system (assoc-in system-config [system/defs :ring :jetty-adapter :conf :handler] api/app))))
