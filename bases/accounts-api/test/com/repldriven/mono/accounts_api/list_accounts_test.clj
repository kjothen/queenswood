(ns ^:eftest/synchronized com.repldriven.mono.accounts-api.list-accounts-test
  (:require
...
(deftest list-accounts-test
  (with-test-system
   [sys
    ["classpath:accounts-api/list-accounts-test.yml"
       ...
       (testing "returns empty when no accounts match"
         (let [after-cursor (cursor/encode "acct-999")]
           (nom-test> [res (list-accounts-request (str "page[after]="
                       ...
                       _ (is (= 200 (:status res)))
                       body (http/res->body res)
                       _ (is (= 0 (count (get body "accounts"))))])))))))
