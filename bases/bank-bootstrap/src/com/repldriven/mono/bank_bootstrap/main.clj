(ns com.repldriven.mono.bank-bootstrap.main
  (:require
...
(defn -main
  ...
  (let [{:keys [options exit-message ok?]}
        (cli/validate-args "bank-bootstrap" args)]
    (if exit-message
      ...
      (let [{:keys [config-file profile]} options
        ...
        (if (error/anomaly? sys)
          (cli/exit false sys)
          (do (log/info "Bootstrap complete; stopping system")
              (system/stop sys)
              (cli/exit true "Bootstrap complete")))))))
